...
describe('RegisterComponent', () => {
  ...
  it('should display a form to register', () => {
    ...
    const loginCtrl = userForm.controls.login;
    ...
    const birthYearCtrl = userForm.controls.birthYear;
    ...
    const passwordForm = userForm.controls.passwordForm;
    expect(passwordForm).withContext('Your form should have a `passwordForm` form group').not.toBeNull();
    expect(passwordForm.valid).toBe(false);
    const passwordCtrl = passwordForm.controls.password;
    ...
    const confirmPasswordCtrl = passwordForm.controls.confirmPassword;
  ...
  it('should have a password confirmation', () => {
    ...
    // given a form
    const passwordForm = fixture.componentInstance.userForm.controls.passwordForm;
    expect(passwordForm.valid).toBe(false);
    const passwordCtrl = passwordForm.controls.password;
  ...
  it('should have min/max validators to check the year validity', () => {
    ...
    // then we should have an error
    expect(birthYearCtrl.hasError('min')).withContext('The birth year control should have a `min` validator set to 1900').toBe(true);
    ...
    expect(birthYearCtrl.hasError('max'))
      .withContext('The birth year control should have a `max` validator set to the next year')
      .toBe(true);
    ...
    // then we should have no error
    expect(birthYearCtrl.hasError('min')).withContext('The birth year control should have a `min` validator set to 1900').toBe(false);
    invalidYearError = nativeElement.querySelector('#invalid-year-error');
    expect(birthYearCtrl.hasError('max'))
      .withContext('The birth year control should have a `max` validator set to the next year')
      .toBe(false);
  ...
  it('should call the user service to register', () => {
    ...
    const userForm = fixture.componentInstance.userForm;
    const loginCtrl = userForm.controls.login;
    loginCtrl.setValue('Cédric');
    const passwordCtrl = userForm.controls.passwordForm.controls.password;
    passwordCtrl.setValue('password');
    const birthYearCtrl = userForm.controls.birthYear;
    birthYearCtrl.setValue(1986);
